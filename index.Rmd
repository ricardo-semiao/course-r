...
tabulate_funs <- function(chap, cat) {
  readxl::read_excel("assets/functions_list.xlsx") %>%
  filter(Chapter == chap & Category == cat) %>%
  select(-c(Chapter, Category)) %>%
  setNames(c("Função", "Descição", "Uso")) %>%
  kable()
}
```
