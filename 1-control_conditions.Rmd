...
- Ao usar um loop para criar uma variável, crie o "conteiner" de antemão, pois é bem mais rápido:
...
- Prefira usar `seq_along(x)`{.r} no lugar de `1:legth(x)`{.r}
...
### Outros loops
Se você não tem um veror de opções de antemão, não sabe sobre quais valores quer iterar, você pode:
