...
- _Logicals_ são os dados booleanos, podem ser `TRUE`{.r} ou `FALSE`{.r}. Podem ser abreviados para `T`{.r} e `F`{.r}\footnote[frame]{Mas as estas não são palavras reservadas}\pause
- _Characters_ são os dados de texto (strings)\pause
- _Intergers_ são números inteiros, e são escritos com um `L`{.r} no fim\pause
...
- Por trás dos panos, existe um `NA`{.r} diferente para cada tipo de vetor atômico (`NA_interger_`{.r}, etc.)
...
Existe uma ordem de prioridade: se houver um _character_, tudo vira _character_, fora isso, tudo vira _double_, depois, _interger_, e por fim, _logical_\footnote[frame]{Atenção: causa comum de erros}
...
::: {.example}
Analize, desenhe, e compare os objetos abaixo:
\vspace{-1em}
```{r}
...
- _dim_ (diminutivo de _dimentions_), um vetor interger, que reorganiza vetores em matrizes e arrays ("matrizes multidimensionais")
...
### Classes
Outro attributo importante é _class_, que permite a criação de vetores diferenciados, criados em cima dos tipos básicos\medskip
...
- _factor_ são intergers para dados categóricos
...
#### Factors
_Factors_ tem o atribuito class como `"factor"`{.r}, e um atributo _levels_ que define os valores/categorias possíveis
...
- Funções criadoras `c()`{.r}, `list()`{.r}, `matrix()`{.r}, `array()`{.r}, `factor()`{.r}, `odered()`{.r}, `data.frame()`{.r}, `tibble()`{.r}, `unlist()`{.r}
...
- Agora com operações aritmétricas. 1 + "2" funciona? e TRUE + 1? Qual é a regra?
...
- O que `dim()`{.r} retorna quando applicado a um vetor unidimensional? Quando `NROW()`{.r} e `NCOL()`{.r} podem ser úteis?
