...
- Seu _enviroment_, um objeto que determina onde a função deve procurar os valores das variáveis relevantes\footnote[frame]{Mais sobre em breve}
...
- **Infix**: a função vai no meio de seus argumentos, `x + y`
  - Modo _prefix_: `` `+`(x, y) ``
  - Para definir use um nome que começe e termine com "%": `%&% <- \(x, y) paste(x, y)`\pause
...
::: block
Um _enviroment_ é como uma lista nomeada, que associa nomes de variáveis aos seus valores. São neles que as funções buscarão os valores das variáveis
:::

**Ambientes são objetos** (como tudo que existe no R), com tipo "enviroment", onde:
...
::: block
No R, as funções salvam o ambiente onde foram criadas como parte de si, "they enclose their enviroments", são **closures**
